  ...
  Streaming(), nh_(nh), frame_id_("World"), valid_(false)
{
  ...
  if (data_format == "image") {
    ...
    if (io_type_ == StreamIOType::Input) {
      ...
      data_format_ = RosDataFormat::LiDAR;
      if (io_type_ == StreamIOType::Input) {
      ...
      data_format_ = RosDataFormat::PC2;
      if (io_type_ == StreamIOType::Input) {
      ...
      if (io_type_ == StreamIOType::Input) {
          //TODO setting pointcloud2 e.g.velodyne inputs
      } else {
          publishers_.push_back(nh_.advertise<sensor_msgs::PointCloud2>(topic_name_, queue_size_));
      }
  }
...
{
  // TODO delete here at last
  if (tag == "lo-test"){
    if (subframe_id_ == "Lidar"){
      if (data_format_ == RosDataFormat::Odometry) {
        CHECK_NOTNULL(tranform_broadcaster_);
        LOG(INFO) << "subframe id :" << subframe_id_;
...
// Send Lidar output data (current scan) to ROS topic for visualization
...
void RosStream::lasermapOutputCallback(DataCluster::Laser_map& map)
{
  if (data_format_ == RosDataFormat::PC2) {
...
void RosStream::livoxCallback(const livox_ros_driver::CustomMsg::ConstPtr& msg){
    ...
    for (size_t i = 1; i < size; i++) {
      if (((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00) && \
          (pow(msg->points[i].x,2)+pow(msg->points[i].y,2)+pow(msg->points[i].z,2) > 1) && \
        ...
        point.curvature = msg->points[i].offset_time * 1e-9; // unit: ms
        point.normal_z = 0; // use 'normal_z' to segment points with default 0. 1 for dynamic, 2 for static.
        valid_num ++ ;
    ...
    data_cluster->lidar->cloud_ptr.reset(new Cloud(scan));
